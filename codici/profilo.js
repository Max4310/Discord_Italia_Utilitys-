...
function profilo(interaction)
{
    try{
        switch(interaction.customId.split(":")[1])
        {
            ...
            case "social": //
                interaction.deferUpdate()
                var social = ["897375656470519829","897375649340207105","897375543446614067","897375658659938346","897375653425467472","897375651865194556","897375655245774848"]
                ...
                if(interaction.values.length != social.length)
                {
                    for(var i in interaction.values)
                    {
                        ...
                        var index = social.indexOf(interaction.values[i])
            ...
            case "notifiche": //
                interaction.deferUpdate()
                const notifiche = ["1062070651487256608", "898331041893343282","944596815708307486","944595871717265438","904306184268419102","916634967130378270","901367919458533376","1028651951740682272","955518863573930094","955518884310564994","1002555397296627782"]
                ...
                if(interaction.values.length != notifiche.length)
                {
                    ...
                    {
                        interaction.member.roles.add(interaction.guild.roles.cache.get(interaction.values[i]))
                        var index = notifiche.indexOf(interaction.values[i])
                        notifiche.splice(index,1)       
                    }
                    for(var x in notifiche)
